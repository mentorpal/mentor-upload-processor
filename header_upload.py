...
def handler(event, context):
    ...
    image_path = ""
    if org:
        image_path = f"images/{org["_id"]}/{datetime.utcnow().strftime('%Y%m%dT%H%M%SZ')}/header.png"
    else:
    ...
    if org:
        ...
        org_header_update(
            OrgHeaderUpdateRequest(imgPath=image_url),
            auth_headers,
        )
    data = {
