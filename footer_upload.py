...
def handler(event, context):
    ...
    if "idx" not in img_request:
        data = {
            "error": "Bad Request",
            "message": "idx parameter missing",
        }
        return create_json_response(401, data, event)
    org = None
    if "org" in img_request:
        org = img_request["org"]
    idx = img_request["idx"]
    ...
    if org is None:
        ...
        orgId = org["_id"]
        image_path = f"images/{orgId}/{datetime.utcnow().strftime('%Y%m%dT%H%M%SZ')}/footer.png"
